...
export function AdminPlansTab({ plans: initialPlans, users, t }: AdminPlansTabProps) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: { original: SubscriptionPlan } }) => formatPrice(row.original.price_weekly)
    },
    {
      id: 'price_biweekly',
      header: 'Precio Quincenal',
      cell: ({ row }: { row: { original: SubscriptionPlan } }) => formatPrice(row.original.price_biweekly)
    },
    {
      id: 'price_monthly',
    ...
    {
      id: 'price_quarterly',
      header: 'Precio Trimestral',
      cell: ({ row }: { row: { original: SubscriptionPlan } }) => formatPrice(row.original.price_quarterly)
    },
    {
      id: 'is_active',
      header: 'Estado',
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Precio Semanal (en pesos)</Label>
                <Input 
                  type="number"
                  value={formData.price_weekly}
                  onChange={(e) => setFormData(prev => ({...prev, price_weekly: Math.max(0, parseInt(e.target.value) || 0)}))}
                />
              </div>
              <div>
                <Label>Precio Quincenal (en pesos)</Label>
                <Input 
                  type="number"
                  value={formData.price_biweekly}
                  onChange={(e) => setFormData(prev => ({...prev, price_biweekly: Math.max(0, parseInt(e.target.value) || 0)}))}
                />
              </div>
              <div>
                <Label>Precio Mensual (en pesos)</Label>
                <Input 
                  type="number"
                  step="0.01"
                  value={formData.price_monthly}
                  onChange={(e) => setFormData(prev => ({...prev, price_monthly: Math.max(0, parseFloat(e.target.value) || 0)}))}
                  required
                />
              </div>
              <div>
                <Label>Precio Trimestral (en pesos)</Label>
                <Input 
                  type="number"
                  step="0.01"
                  value={formData.price_quarterly}
                  onChange={(e) => setFormData(prev => ({...prev, price_quarterly: Math.max(0, parseFloat(e.target.value) || 0)}))}
                  required
                />
              </div>
            </div>
